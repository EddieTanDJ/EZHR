...
class ManagerClaimsViewModel : ViewModel() {
    ...
    /**
     * Retrieve and display all pending claim applications from firebase database
     */
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    for (userSnapshot in snapshot.children) {
                        if (userSnapshot.child("status").getValue(String::class.java) == "PENDING"
                        ) {
...
class ManagerClaimsViewModelFactory : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(ManagerClaimsViewModel::class.java)) {
            return ManagerClaimsViewModel() as T
        }
